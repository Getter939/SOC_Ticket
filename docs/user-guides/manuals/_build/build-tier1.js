...
push(H2("5.4 หลังส่งต่อให้ Tier 2 และเมื่อผู้จัดการส่งกลับ"));
push(P([r("เมื่อส่งต่อให้ Tier 2 แล้ว "), r("Tier 2 เป็นผู้ตัดสินและเลือกเส้นทางการจัดการเอง", { bold: true }),
  r(" แล้วส่งผู้จัดการ SOC ตรวจทันที คุณไม่ต้องดำเนินการต่อ")]));
push(step("s54", [r("ติดตามสิ่งที่ Tier 2 แก้ไขในตั๋วที่คุณเปิด ได้ที่แท็บ "), ui("Tier 2 แก้ไข"), r(" ใน "), menuTag("คิวงาน Tier 1"),
  r(" (แจ้งให้ทราบเท่านั้น รายการจะหายไปเมื่อคุณเปิดดูตั๋ว)")]));
push(step("s54", [r("หากคุณส่งตั๋ว Incident ตรงจากขั้นจัดเตรียมไปยังผู้จัดการ แล้วผู้จัดการ "), r("ส่งกลับ", { bold: true }),
  r(" ตั๋วจะกลับมาที่ "), r("กำลังจัดเตรียม (ยังไม่ส่ง)", { bold: true }), r(" พร้อมเหตุผล — แก้ไขให้ครบ แล้วกด "),
  ui("ส่ง Ticket เข้าสู่กระบวนการ"), r(" อีกครั้ง")]));
...
push(H2("5.6 เมื่อเคสอยู่ในการเฝ้าระวัง (Monitoring)"));
push(P([r("เมื่อ Tier 2 จัดเคสที่ถูกส่งต่อเป็น "), r("Event", { bold: true }), r(" และเห็นควรเฝ้าดูก่อนปิด เขาจะกด “เฝ้าระวัง 30 วัน” เคสจะกลับมาอยู่กับคุณ (ผู้เปิดตั๋ว) ที่สถานะ "), r("กำลังเฝ้าระวัง", { bold: true }), r(" และปรากฏในแท็บ "), ui("กำลังเฝ้าระวัง"), r(" ของ "), menuTag("คิวงาน Tier 1"), r(" พร้อมตัวนับถอยหลัง (เขียว → เหลืองเมื่อใกล้ครบ → แดงเมื่อครบกำหนด)")]));
push(P([r("บทบาทของคุณในช่วงนี้คือ "), r("เฝ้าติดตามเพื่อการมองเห็น", { bold: true }), r(" — แนบหลักฐานที่พบระหว่างทางได้ แต่ "), r("ผู้สรุปผลการเฝ้าระวังคือ Tier 2", { bold: true }), r(" เมื่อครบกำหนดหรือมีเหตุเกิดขึ้น Tier 2 จะปิดเป็น Event หรือยกระดับเป็น Incident เอง หากคุณสังเกตเห็นเหตุผิดปกติ ให้แจ้ง Tier 2 เพื่อพิจารณายกระดับ")]));
push(callout("note", ["เฝ้าระวังได้ครั้งเดียวต่อเคส (30 วันคือกำหนดสูงสุด ขยายไม่ได้) เฉพาะเคสที่เป็น Event เท่านั้น (Incident เฝ้าระวังไม่ได้) และระหว่างเฝ้าระวังยังตั้งแฟล็กฉุกเฉินไม่ได้"]));
...
push(dataTable(
  ...
  [
    [stateName("กำลังจัดเตรียม (ยังไม่ส่ง)", "NEW"), "ตั๋วถูกบันทึกไว้แล้วแต่ยังไม่ส่งเข้ากระบวนการ (หรือผู้จัดการส่งกลับมาให้แก้ไข)", [r("เติมข้อมูล/หลักฐานให้ครบ แล้วกด ", {}), r("ส่ง Ticket เข้าสู่กระบวนการ", { bold: true }), r(" (ดู 5.3.2)", {})]],
    [stateName("ส่งต่อให้ Tier 2", "ESCALATED_T2"), "คุณส่งให้ Tier 2 พิจารณา", "รอ Tier 2 — ยังไม่ต้องทำอะไร"],
    [stateName("กำลังเฝ้าระวัง", "MONITORING"), "Tier 2 จัดเป็น Event แล้วเฝ้าระวัง 30 วัน", "เฝ้าติดตามเพื่อการมองเห็น — Tier 2 เป็นผู้สรุปผล (ดู 5.6)"],
...
push(H3("ปิดคำขอทีมตอบสนองแทนทีมตอบสนองได้ไหม?"));
push(P("ไม่ได้ — การเปลี่ยนสถานะของคำขอ (เช่น ปิดเป็น “เสร็จสิ้น”) และการระบุเลขที่รายงาน ทำได้เฉพาะผู้รับผิดชอบคำขอหรือผู้จัดการ SOC เพราะการปิดคำขอคือสิ่งที่ปลดล็อกให้อนุมัติปิดเคสได้ คุณกด “อัปเดตงาน” ในหัวข้อคำขอทีมตอบสนองเพื่อเพิ่มหมายเหตุได้ แต่ช่องสถานะจะล็อกไว้ หากคำขอค้างนาน ให้ประสานผู้จัดการ SOC"));
...
push(dataTable(
  ...
  [
    ...
    [[r("คำขอทีมตอบสนอง", { bold: true })], "งานเฉพาะทางที่ผู้จัดการส่งให้ทีมนอก SOC (พิสูจน์หลักฐาน/ทดสอบเจาะระบบ) ปิดได้เฉพาะผู้รับผิดชอบคำขอหรือผู้จัดการ SOC (Tier 1 เพิ่มหมายเหตุได้)"],
  ],
  [3000, 6600],
));
