...
cover.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 },
  children: [r("เวอร์ชันระบบ v1.7.8", { color: INK, size: 28 }), r("   ·   ", { color: HAIR, size: 28 }),
             r("ปรับปรุงล่าสุด 25 กันยายน 2026", { color: INK, size: 28 })] }));
cover.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 },
...
// 4.1 — the shared list filter bar + sortable headers (v1.7.8). The other
// manuals get the same body from common.js listControls(); this script keeps
// an inline copy because it predates common.js.
push(H2("4.1 ค้นหา กรอง และเรียงลำดับรายการ (ปรับปรุง v1.7.8)"));
push(P("ทุกหน้าที่เป็นรายการใช้แถบค้นหาและการเรียงลำดับแบบเดียวกัน:"));
push(bullet([r("ค้นหา: ", { bold: true }), r("พิมพ์คำค้นแล้วกด Enter หรือกดปุ่มแว่นขยาย โดยทั่วไปค้นได้จากเลข Ticket ชื่อเรื่อง ระบบ/บริการ IP และรายละเอียด")]));
push(bullet([r("ตัวเลือกกรอง ", { bold: true }), r("เช่น ความรุนแรง หรือสถานะ มีผลทันทีที่เลือก "), r("ไม่มีปุ่ม “กรอง” แล้ว", { bold: true })]));
push(bullet([r("ป้ายตัวกรอง: ", { bold: true }), r("ทุกเงื่อนไขที่ใช้อยู่แสดงเป็นป้ายใต้แถบ กด × บนป้ายเพื่อเอาเงื่อนไขนั้นออก หรือกด "), ui("ล้างทั้งหมด"),
  r(" ช่องที่กำลังกรองอยู่จะมีกรอบสีน้ำเงิน — ถ้ารายการดูสั้นผิดปกติ ให้ดูป้ายก่อน")]));
push(bullet([r("ตัวกรองเพิ่มเติม: ", { bold: true }), r("ในบางหน้า ตัวกรองที่ใช้ไม่บ่อยซ่อนอยู่ใต้ปุ่ม "), ui("ตัวกรองเพิ่มเติม"),
  r(" ตัวเลขบนปุ่มบอกว่าตั้งไว้กี่รายการ และส่วนนี้จะเปิดค้างไว้เองเมื่อมีรายการที่ตั้งอยู่")]));
push(bullet([r("แถบผลลัพธ์ ", { bold: true }), r("เหนือตารางบอกจำนวนที่พบ เช่น “13 เคส จาก 89 ที่เปิดอยู่” และมีตัวเลือก "), ui("เรียงตาม"), r(" แบบสำเร็จรูป")]));
push(bullet([r("กดหัวคอลัมน์ ", { bold: true }),
...
push(bullet([r("ลิงก์ ", { bold: true }), r("ของป้าย หน้าถัดไป และหัวคอลัมน์ คงเงื่อนไขทั้งหมดไว้ คัดลอกที่อยู่ในเบราว์เซอร์ส่งให้เพื่อนร่วมทีมเพื่อเปิดมุมมองเดียวกันได้")]));
push(spacer(80));
push(dataTable(
  ["หน้า", "ตัวกรองบนแถบ และข้อสังเกต"],
  [
    [[menuTag("คิวงาน Tier 1"), r(" (คิวงานของฉัน)", { size: 28 })],
     [r("ช่องค้นหาเดียวเหนือแท็บ กรองทุกแท็บงาน (Ticket ของฉัน, กำลังเฝ้าระวัง, Tier 2 แก้ไข, รับแจ้งด้วยตนเอง) และตัวเลขบนแท็บ — แท็บ ที่ฉันดำเนินการล่าสุด และกล่องแจ้งเตือนด้านบนไม่ถูกกรอง แต่ละแท็บเรียงตามหัวคอลัมน์ของตัวเอง และเมื่อหน้าโหลดใหม่จะกลับมาที่แท็บเดิม", { size: 28 })]],
    [[menuTag("คัดกรองการแจ้งเตือนจาก Wazuh")],
     [r("ป้าย งาน / OLA / ระดับความอันตรายจาก Wazuh (ตัวเลขบนป้ายคือจำนวนที่จะเห็นเมื่อกด) ช่องค้นหา และ ต่อหน้า 25 / 50 / 100 ในแถบผลลัพธ์ — ดูหัวข้อ 5.1", { size: 28 })]],
    [[menuTag("เคสที่กำลังดำเนินการอยู่")],
     [r("ค้นหา สถานะ ความรุนแรง และ วันที่แจ้ง — ใต้ ", { size: 28 }), ui("ตัวกรองเพิ่มเติม"),
    ...
    [[menuTag("ประวัติเคสที่ปิดไปแล้ว")],
     [r("ป้ายผลลัพธ์ ทั้งหมด / อนุมัติแล้ว / Event / ยกเลิกแล้ว พร้อมจำนวน, ค้นหา ความรุนแรง ผู้อนุมัติ/ปิดเคส และ วันที่แจ้ง — ใต้ ", { size: 28 }), ui("ตัวกรองเพิ่มเติม"),
...
push(spacer(80));
push(...shot("แถบค้นหาและกรองเหนือรายการเคส พร้อมป้ายตัวกรอง แถบผลลัพธ์ และหัวคอลัมน์ที่กดเรียงได้", "T1-list-filter-bar", 4));
...
push(step("s51", [r("เปิดเมนู "), menuTag("คัดกรองการแจ้งเตือนจาก Wazuh"), r(" จะเห็นรายการ Alert เรียงตาม OLA ใกล้ครบกำหนดก่อน")]));
push(step("s51", [r("ปรับมุมมองด้วยแถบด้านบน (ดูหัวข้อ 4.1): ป้าย "), ui("งาน"), r(", "), ui("OLA"), r(" และ "), ui("ระดับความอันตรายจาก Wazuh"),
  r(" — ตัวเลขบนป้ายคือจำนวนที่จะเห็นเมื่อกด — ช่อง "), ui("ค้นหา"), r(" (Agent, IP, Rule ID, MITRE ID, รายละเอียด) และ "),
  r("กดหัวคอลัมน์", { bold: true }), r(" เพื่อเรียงลำดับได้ทุกคอลัมน์ ยกเว้น OLA และ ดำเนินการ ส่วน "), ui("เรียงตาม"), r(" และ "), ui("ต่อหน้า"),
  r(" ("), r("25 / 50 / 100", { bold: true }), r(") อยู่ในแถบผลลัพธ์เหนือตาราง — แถวที่ไม่มีค่าในคอลัมน์ที่เรียง จะถูกจัดไว้ท้ายสุดเสมอ")]));
...
push(P("ผลลัพธ์แสดงเป็นสองการ์ดที่แยกหน้ากัน แต่ละการ์ดกดหัวคอลัมน์เพื่อเรียงได้เอง (ปรับปรุง v1.7.8) — เรียงการ์ดหนึ่งไม่ทำให้อีกการ์ดเปลี่ยนลำดับหรือกลับไปหน้าแรก:"));
push(dataTable(
  ["การ์ด", "ค้นจากอะไร"],
  [
...
push(H3("ปุ่ม “กรอง” หายไปไหน?"));
push(P("ตั้งแต่ v1.7.8 ตัวเลือกกรองมีผลทันทีที่เลือก และช่องค้นหาทำงานเมื่อกด Enter จึงไม่มีปุ่มกรองแล้ว เงื่อนไขที่ใช้อยู่ทั้งหมดแสดงเป็นป้ายใต้แถบ กด × เพื่อเอาออก (ดู 4.1)"));
push(H3("ทำไมเคสที่ปิดเดือนนี้ไม่อยู่ใน “ประวัติเคสที่ปิดไปแล้ว”?"));
push(P("ช่วงวันที่ของหน้านั้นนับจากวันที่แจ้ง ไม่ใช่วันที่ปิด และค่าเริ่มต้นคือเคสที่แจ้งในเดือนนี้ เคสที่เปิดเดือนก่อนจึงไม่อยู่ในมุมมองเริ่มต้นแม้จะปิดเดือนนี้ — เลือก “ทุกช่วงเวลา” ในช่องวันที่แจ้ง หรือขยายช่วงวันที่ (ดู 4.1)"));
