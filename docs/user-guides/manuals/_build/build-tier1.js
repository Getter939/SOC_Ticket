...
cover.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 },
  children: [r("เวอร์ชันระบบ v1.5.0", { color: INK, size: 28 }), r("   ·   ", { color: HAIR, size: 28 }),
             r("ปรับปรุงล่าสุด 11 กันยายน 2026", { color: INK, size: 28 })] }));
cover.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 },
  children: [r("คู่มือฉบับที่ 1.2 (ตัวอย่าง)", { color: MUTED, size: 28 })] }));
...
push(dataTable(
  ...
  [
    ...
    [[menuTag("Intake & Triage › My Queue"), r(" (คิวงานของฉัน)")], "งานที่รอคุณดำเนินการ, เคสที่กำลังเฝ้าระวัง, รายการรับแจ้งด้วยตนเอง, และประวัติ"],
...
push(step("s51", [r("ปรับมุมมองได้ตามต้องการ: "), r("กดหัวคอลัมน์", { bold: true }),
  r(" (เวลา, Agent, Agent IP, ระดับ, Rule, สถานะ, ผู้ดูแล) เพื่อเรียงลำดับ และเลือกจำนวนแถวต่อหน้าได้ "),
...
push(callout("tip", [
  ...
  [r("ค่าที่กรอกไว้จะเข้าสู่ ", {}), menuTag("IOC Search"), r(" ประวัติการแก้ไขของตั๋ว และรายงานเหตุการณ์ รวมทั้งไปปรากฏใน ", {}), menuTag("IOC Database"), r(" ให้ทีมพิสูจน์หลักฐานตรวจต่อ — กรอกให้ครบเท่าที่มีจึงมีประโยชน์กับทั้งกระบวนการ", {})],
]));
...
push(P([r("เมื่อ Tier 2 เห็นว่าเคสยังไม่ชัดว่าเป็น Event หรือ Incident เขาจะกด “เฝ้าระวัง 30 วัน” เคสจะกลับมาอยู่กับคุณ (ผู้เปิดตั๋ว) ที่สถานะ "), r("กำลังเฝ้าระวัง", { bold: true }), r(" และปรากฏในแท็บ "), ui("กำลังเฝ้าระวัง"), r(" ของ "), menuTag("My Queue"), r(" พร้อมตัวนับถอยหลัง (เขียว → เหลืองเมื่อใกล้ครบ → แดงเมื่อครบกำหนด)")]));
...
push(step("s58", [r("ขณะสถานะยังเป็น "), r("แจ้งเหตุใหม่ (NEW)", { bold: true }), r(" เท่านั้น คุณ (ผู้เปิดตั๋ว) กด "),
...
function numberedRefs() {
  const refs = ["s3", "s51", "s52", "s53", "s54", "s55", "s56", "s56b", "s58"];
